...
# get model
def get_model():
    return pd.read_pickle('model_01:35:21.659075_125.38.pkl')
    # X = df
    #
    # X = sm.add_constant(X)
    # y = df['price']
...
def get_csv_dataframe():
    ...
    return pd.read_csv('../data/AirBNB_mystery.csv', header=0)
...
def add_missing_cols(dfTrainTestPrep, dfProduction):
    ...
    print(len(trainTestColumns), trainTestColumns)
    productionColumns = list(dfProduction.keys())
    print(len(productionColumns), productionColumns)
    count = 0
    not_in = 0
...
def clean_data():
    ...
    print(X_mystery.head(5))
    print('=' * 50)
    X_mystery = sm.add_constant(X_mystery, has_constant='add')
    print(X_mystery.head(5))
...
# Predict
def predict_data(model, X_mystery):
    X_mystery = sm.add_constant(X_mystery)
    predicted = model.predict(X_mystery)
    print(predicted)
...
def test_predictions(X_mystery):
    predictions = []
    ...
    # X_mystery["predicted_price"] = predictions
    #
    # print(X_mystery[["predicted_price"]].head(10))
    return predictions
...
def main():
    ...
    # print(df_to_predict.head(10))
    # print(df_to_predict.keys())
    # print(len(df_to_predict.keys()))
    predict_data(model, X_mystery)
    print("=" * 100)
    test_predictions(X_mystery)
